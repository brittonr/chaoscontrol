# Napkin
| 2026-04-01 | self | Rare-edge scoring, frontier recycling, and havoc mutations added | score_branch() now weights rare edges (global count ≤3) at 5× per edge. Frontier recycling re-seeds from corpus when frontier empties. Havoc mode (4-16 mutations/variant) activates at stale_round_limit/2. |
| 2026-04-01 | self | assert_raw() was already implemented — napkin was stale | Functions, macro, enum, no-op stubs, prelude exports, and tests all existed. Updated napkin. |
| 2026-04-01 | self | Interactive debugger added (inspect subcommand) | REPL: goto/step/rewind, regs/mem/serial, poke/setreg/whatif. Live modifications + counterfactual replay. Register Display/FromStr added. |
...
| 2026-03-31 | self | Coverage plateaus after round 1 for Raft with 1000-tick branches | FIXED: assertion-state enrichment extends useful rounds from 1→3 (15K→29K edges). Stale-round early stop (default 10) halts at round 13 instead of grinding 50 empty rounds. Saved 74% wall-clock. Upper half of 64K bitmap reserved for assertion-state hashing (verdict + hit-bucket + ratio-bucket per assertion ID). |
