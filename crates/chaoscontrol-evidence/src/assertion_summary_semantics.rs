...
pub(crate) fn validate_common(assertion: AssertionSemantics<'_>) -> crate::EvidenceResult<()> {
    for (field, value) in [
    ...
    if assertion.message.len() > ::chaoscontrol_protocol::identity::MAX_ASSERTION_MESSAGE_BYTES
        || assertion.guest.len() > ::chaoscontrol_protocol::identity::MAX_ASSERTION_GUEST_BYTES
        || assertion.category.len()
    ...
    let counted = assertion
        ...
        .ok_or_else(|| crate::EvidenceError::new("assertion-summary: counter overflow"))?;
    ...
    if assertion.last_failure_details.is_some_and(|details| {
...
fn expected_verdict(assertion: &AssertionSemantics<'_>) -> crate::EvidenceResult<&'static str> {
