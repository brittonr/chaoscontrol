...
fn main() {
    ...
    for num_vcpus in [4, 8] {
        ...
        vm.load_kernel("result-dev/vmlinux", Some("guest/initrd.gz")).expect("load kernel");
        ...
        for chunk in 0..30 {
            ...
            if output.contains("Brought up") || output.contains("login:") || halted {
                println!("  Completed at chunk {} (total exits={})", chunk, vm.exit_count());
                break;
            }
            if chunk % 5 == 4 {
        ...
        for line in output.lines() {
            ...
            if l.contains("cpu") || l.contains("smp") || l.contains("brought") || l.contains("apic") || l.contains("processor") {
                println!("  | {}", line.trim());
            }
        }
