...
| Operator triage UX | `local-runbook` | Current evidence includes a committed local operator triage runbook generated from replay-readiness receipts and accepted proof artifacts, but not a hosted triage workflow or fleet UI. | Hosted or UI-backed triage evidence that starts from a readiness receipt, opens the bug/replay artifacts, runs reproduce/minimize, and records operator decisions without raw-log scraping. |
