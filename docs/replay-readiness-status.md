...
| Hosted/fleet triage UI | `missing-hosted-fleet-ui` | The current dashboard is a static single-receipt artifact and the runbook is local-only; there is no hosted multi-run UI, shared decision store, fleet scheduler integration, or operator workflow evidence across machines. | Hosted or UI-backed fleet triage evidence that ingests readiness receipts from multiple runs, links bug/replay artifacts, runs or records reproduce/minimize outcomes, persists operator decisions, and proves the workflow without raw-log scraping. |
