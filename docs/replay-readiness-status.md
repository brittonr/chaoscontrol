...
| Replay scheduler orchestration | `local-scheduler-receipts` | Current evidence includes a bounded local scheduler receipt for a manual multi-run plan, but not a hosted scheduler, shared queue, cross-machine orchestration, or automatic campaign service. | A scheduler integration that executes or records multiple replay-readiness runs, links each run to receipt artifacts and local decisions, enforces bounded concurrency/failure behavior, and proves the workflow without raw-log scraping or shared-queue overclaims. |
