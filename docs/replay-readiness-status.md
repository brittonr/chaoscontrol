...
| Replay scheduler orchestration | `bounded-fleet-scheduler-runtime` | Current evidence includes bounded local sequential scheduler execution plus a bounded local hosted/fleet worker loop that consumes a durable queue plan, leases work to workers, executes replay-readiness commands, and emits run-linked receipt summaries. It is still not a multi-machine hosted campaign service. | A multi-machine hosted scheduler integration that persists queue state across service restarts, links each run to receipt artifacts and local decisions, enforces bounded concurrency/failure behavior across workers, and proves the workflow without raw-log scraping. |
