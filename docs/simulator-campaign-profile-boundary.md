...
Use `check-profile-admission <kind> <path>` to test the Rust boundary without starting a VM, thread, or simulator. Rust uses bounded regular-file reads, closed serde shapes, and repeated safety checks.
