...
The UI SHALL display bug discoveries as markers on the coverage chart or as a separate timeline. Each marker SHALL show the round number and assertion message on hover.

#### Scenario: Bug marker on chart
- **WHEN** a bug is discovered at round R
- **THEN** a vertical marker or annotation appears at round R on the coverage chart

#### Scenario: Bug hover detail
- **WHEN** the user hovers over a bug marker
- **THEN** a tooltip shows the assertion message, assertion kind, round number, and tick

### Requirement: Assertion status table
The UI SHALL display a table of all registered assertions with columns: status icon (✓/✗/○), message, kind, hit count, and verdict. Failed assertions SHALL appear first, then unexercised, then passed.

#### Scenario: Failed assertion row
- **WHEN** an assertion has verdict "failed"
- **THEN** its row shows a ✗ icon, red background tint, and the hit count ratio (true_count/hit_count)
...
The UI SHALL use a dark background color scheme suitable for terminal-adjacent tooling.

#### Scenario: Page load appearance
- **WHEN** the dashboard loads
- **THEN** the page uses a dark background (#1a1a2e or similar) with light text and accent colors for charts
